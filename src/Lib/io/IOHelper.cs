...
namespace Lib.io
{
    ...
    public sealed class IOHelper
    {
        ...
        {
            var di = new DirectoryInfo(directory ?? throw new ArgumentNullException(nameof(directory)));
        ...
        public static string GetFileExtension(string ContentType, string FileName)
        {
            ...
            //使用contentType
            if (ValidateHelper.IsNotEmpty(ContentType))
            {
                var sp = ContentType.Split('/').Where(x => ValidateHelper.IsNotEmpty(x)).ToArray();
                if (sp.Length == 2)
                {
                    ext = "." + sp[1];
                }
            ...
            //使用文件名
            if (ValidateHelper.IsNotEmpty(FileName) && FileName.IndexOf('.') >= 0)
            {
                /*
