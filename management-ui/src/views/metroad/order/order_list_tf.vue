<template>
  <div>
    <a-card size="small" title="投放列表" :headStyle="{backgroundColor:'#f9dfe0'}">
      <span slot="extra">
        <router-link :to="{name:'metroad-order-order-list'}">
          <a-button type="dashed" size="small" icon="left">订单列表</a-button>
        </router-link>
      </span>
      ...
      >
        <span slot="time" slot-scope="text,record">
          ...
          <div v-if="expired_data(record)">
            <a-badge status="error" text="待下架" />
          </div>
        </span>
...
import { status_list, list_columns_tf, tf_tab } from './data'
...
export default {
  ...
  methods: {
    ...
    reset_and_load_data(s) {
      this.$router.push({ name: 'metroad-order-order-list-tf', query: { status: s } })
    },
    ...
    load_data() {
      ...
      delete p.status
      delete p.time_range
      if (!(multi_status.length == 1 && multi_status.indexOf(2) >= 0)) {
        delete p.has_design
      }
      if (!(multi_status.length == 1 && multi_status.indexOf(5) >= 0)) {
        delete p.is_expired
      }
