...
namespace Lib.extension
{
    ...
    {
        public static Dictionary<Key, Value> ToDictionary_<Key, Value>(this IDictionary<Key, Value> dict) =>
        ...
        /// 并发读，同步写
        /// </summary>
        public static Value GetOrSetWithLock<Key, Value>(this IDictionary<Key, Value> dict, Key k, Func<Value> v)
        {
            if (!dict.ContainsKey(k))
                lock (dict)
                    if (!dict.ContainsKey(k))
        ...
        public static string ToUrlParam(this IDictionary<string, string> dict) =>
            string.Join("&", dict.Select(x => $"{x.Key ?? string.Empty}={x.Value ?? string.Empty}"));
